...
管理 Claude Code 会话历史 - 列出、加载、别名管理以及编辑存储在 `~/.claude/sessions/` 中的会话。
...
显示带有元数据、过滤和分页功能的会话列表。

```bash
/sessions                              # 列出所有会话 (默认)
/sessions list                         # 同上
/sessions list --limit 10              # 显示最近 10 个会话
...
console.log('会话列表 (显示 ' + result.sessions.length + ' / ' + result.total + '):');
console.log('');
...
console.log('会话文件: ' + session.filename);
...
if (result.success) {
  console.log('✓ 别名创建成功: ' + aliasName + ' → ' + session.filename);
} else {
...
- 会话以 markdown 文件形式存储在 `~/.claude/sessions/` 目录下
- 别名信息存储在 `~/.claude/session-aliases.json` 文件中
- 会话 ID 可以缩写 (通常前 4-8 个字符就足够唯一标识)
- 建议为经常访问的会话创建别名
