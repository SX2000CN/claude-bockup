...
function validateHooks() {
  ...
  try {
    ...
    console.error(`错误: hooks.json JSON 格式无效: ${e.message}`);
    process.exit(1);
  }
  ...
  if (typeof hooks === 'object' && !Array.isArray(hooks)) {
    ...
    for (const [eventType, matchers] of Object.entries(hooks)) {
      ...
      if (!Array.isArray(matchers)) {
        console.error(`错误: ${eventType} 必须是一个数组`);
        hasErrors = true;
        continue;
      }
      ...
      for (let i = 0; i < matchers.length; i++) {
        ...
        if (typeof matcher !== 'object' || matcher === null) {
          console.error(`错误: ${eventType}[${i}] 不是一个对象`);
          hasErrors = true;
          continue;
        }
        if (!matcher.matcher) {
    ...
    for (let i = 0; i < hooks.length; i++) {
      const hook = hooks[i];
      if (!hook.matcher) {
        console.error(`错误: Hook ${i} 缺少 'matcher' 字段`);
        hasErrors = true;
      }
      if (!hook.hooks || !Array.isArray(hook.hooks)) {
        console.error(`错误: Hook ${i} 缺少 'hooks' 数组`);
        hasErrors = true;
      } else {
        // Validate each hook entry
        for (let j = 0; j < hook.hooks.length; j++) {
          ...
          if (!h.type || typeof h.type !== 'string') {
            console.error(`错误: Hook ${i}.hooks[${j}] 缺少或无效的 'type' 字段`);
            hasErrors = true;
          }
          if (!h.command || (typeof h.command !== 'string' && !Array.isArray(h.command))) {
            console.error(`错误: Hook ${i}.hooks[${j}] 缺少或无效的 'command' 字段`);
            hasErrors = true;
          }
        }
      }
