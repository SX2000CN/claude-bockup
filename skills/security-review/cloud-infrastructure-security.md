| name | description |
|------|-------------|
| cloud-infrastructure-security | 部署到云平台、配置基础设施、管理 IAM 策略、设置日志/监控或实施 CI/CD 管道时使用此技能。提供符合最佳实践的云安全检查清单。 |
...
#### VPC 和防火墙配置 (VPC and Firewall Configuration)
...
#### 安全管道配置 (Secure Pipeline Configuration)
...
### 7. 备份与灾难恢复 (Backup & Disaster Recovery)
...
## 部署前云安全检查清单 (Pre-Deployment Cloud Security Checklist)
...
## 常见云安全错误配置 (Common Cloud Security Misconfigurations)
...
**记住**: 云配置错误是数据泄露的主要原因。单个暴露的 S3 存储桶或过于宽松的 IAM 策略都可能危及整个基础设施。始终遵循最小权限原则和纵深防御。
