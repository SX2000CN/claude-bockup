---
name: django-tdd
description: Django 测试策略，使用 pytest-django, TDD 方法论, factory_boy, mocking, coverage, 以及测试 Django REST Framework APIs。
---

# Django TDD 测试 (Django Testing with TDD)
...
- 编写新的 Django 应用程序
- 实现 Django REST Framework APIs
- 测试 Django 模型、视图和序列化器
- 为 Django 项目设置测试基础设施

## Django TDD 工作流 (TDD Workflow for Django)

### 红-绿-重构 循环 (Red-Green-Refactor Cycle)
...
# 步骤 3: 重构 (REFACTOR) - 在保持测试通过的情况下改进代码
```
...
### Django 视图测试 (Django View Testing)
...
## DRF API 测试 (DRF API Testing)

### 序列化器测试 (Serializer Testing)
...
### API ViewSet 测试 (API ViewSet Testing)
...
### Mocking 外部服务 (Mocking External Services)
...
### Mocking 邮件发送 (Mocking Email Sending)
...
记住：测试即文档。好的测试解释了你的代码应该如何工作。保持它们简单、可读和可维护。
