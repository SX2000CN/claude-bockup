---
name: jpa-patterns
description: Spring Boot 中的 JPA/Hibernate 模式，用于实体设计、关系、查询优化、事务、审计、索引、分页和连接池。
---
...
## 关系与 N+1 问题预防 (Relationships and N+1 Prevention)
...
- 默认为延迟加载 (lazy loading)；需要时在查询中使用 `JOIN FETCH`
...
对于类游标分页 (cursor-like pagination)，在 JPQL 中包含 `id > :lastId` 并排序。

## 索引与性能 (Indexing and Performance)
...
- 使用 Flyway 或 Liquibase；绝不在生产环境依赖 Hibernate 自动 DDL
- 保持迁移幂等且可添加；避免在没有计划的情况下删除列

## 测试数据访问 (Testing Data Access)
...
**记住**: 保持实体精简，查询有意图，事务简短。通过抓取策略和投影防止 N+1 问题，并为读写路径建立索引。
